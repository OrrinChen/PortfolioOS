...
def choose_marginal_value_decision(row: pd.Series) -> tuple[str, str]:
    ...
    if row.get("incremental_turnover", 0.0) >= 0.75 and row["incremental_net_spread"] <= 0:
        return "archive_no_marginal_value", "high-turnover factor has non-positive net marginal value"
